...
class AttributeMetadata(BaseModel):
    ...
    safety_level: str = Field(
        description=(
            "Risk level: 'critical' (life-threatening), 'warning' "
            "(significant restriction), or 'info' (useful guidance)"
        )
    )
    dietary_rules: list[str] = Field(
        description=(
            "Specific dietary rules to follow (e.g. 'Avoid all "
...
class EnrichMetadataAgent:
    """Agent that researches health conditions and allergies.
    ...
    async def enrich_condition(
        ...
        Args:
            ...
            value: The specific condition/allergy name (e.g. 'Celiac Disease',
            'Peanut allergy')
        """
        ...
        prompt = SystemPrompt(
            background=[
                (
                    "You are a clinical nutrition expert with deep knowledge "
                    "of medical dietary requirements."
                ),
                (
                    "Your job is to provide evidence-based, structured "
                    "dietary metadata for specific health conditions and "
                    "allergies."
                ),
            ],
            steps=[
                ...
                (
                    "  - 'critical': Life-threatening (e.g. severe peanut "
                    "allergy, celiac disease)"
                ),
                (
                    "  - 'warning': Significant dietary restriction needed "
                    "(e.g. lactose intolerance, diabetes)"
                ),
                (
                    "  - 'info': Helpful guidance but not dangerous (e.g. "
                    "mild preference, general wellness)"
                ),
            ...
            output=[
                (
                    "Be specific and actionable — list actual food names, "
                    "not vague categories"
                ),
                (
                    "dietary_rules should be clear instructions (e.g. 'Avoid "
                    "all dairy products including milk, cheese, yogurt')"
                ),
                (
                    "foods_to_avoid and foods_to_prioritize should be "
                    "specific items"
                ),
        ...
        try:
            ...
            logger.exception("Failed to enrich %s '%s'", category, value)
            return AttributeMetadata(
                safety_level="info",
                dietary_rules=[],
                foods_to_avoid=[],
                foods_to_prioritize=[],
                general_advice="Could not retrieve metadata.",
            )
        else:
            logger.info(
                "Enriched %s '%s' → safety=%s, rules=%d, avoid=%d, "
                "prioritize=%d",
                category,
                value,
                result.safety_level,
    ...
    async def enrich_member_profile(
        ...
        Args:
            conditions: Health conditions (e.g. ['Diabetes'])
            allergies: Allergies (e.g. ['Peanut'])
        """
        ...
        for (cat, name), result in zip(task_keys, results, strict=True):
            if isinstance(result, Exception):
                logger.error(
                    "Exception enriching %s '%s': %s", cat, name, result
                )
                continue
            if cat == "condition":
